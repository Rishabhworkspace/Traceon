...
import { Clock, GitCommit, Plus, Minus, ArrowLeftRight, ChevronUp, ChevronDown, X } from 'lucide-react';
...
export default function TimelineSlider({ commits, selectedIndex, onChange, diffStats }: TimelineSliderProps) {
    ...
    const uiCommits = useMemo(() => commits ? [...commits].reverse() : [], [commits]);
    const uiSelectedIndex = commits ? (commits.length - 1) - selectedIndex : 0;
    ...
    if (!commits || commits.length === 0 || isClosed) return null;

    return (
        <div className={`absolute bottom-6 left-1/2 -translate-x-1/2 z-40 bg-[#0d0d0d]/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-[0_20px_60px_rgba(0,0,0,0.5)] px-6 py-4 transition-all duration-300 ${isMinimized ? 'w-fit min-w-[300px]' : 'w-full min-w-[500px] max-w-[640px]'}`}>
            {/* Header */}
            <div className={`flex items-center justify-between ${!isMinimized ? 'mb-4' : ''}`}>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        >
                            {isMinimized ? <ChevronUp size={14} className="text-gray-400" /> : <ChevronDown size={14} className="text-gray-400" />}
                        </button>
                        <button
