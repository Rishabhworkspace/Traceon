...
export async function fetchGitHubProfileData(username: string): Promise<EnrichedProfileData> {
    ...
    let page = 1;
    while (page <= 3) { // Max 300 repos (3 pages × 100)
    ...
    let orgsCount = 0;
    try {
        const orgsRes = await safeFetch(`https://api.github.com/users/${username}/orgs`, headers);
        if (orgsRes) {
    ...
    const filteredRepos = filterAndWeightRepos(allRepos as any, readmePresence, licensePresence);
    ...
    for (const chunk of repoChunks) {
        await Promise.all(chunk.map(async (repo) => {
            try {
                await analyzeRepo(repo, username, headers, languageBytes, recentCommits, readmeSnippets, repoQualitySignals, deepAnalysisRepos);
            } catch (error) {
    ...
    const diffMonths = (new Date().getFullYear() - createdAt.getFullYear()) * 12
...
async function analyzeRepo(
    ...
    const langRes = await safeFetch(
        `https://api.github.com/repos/${repo.owner}/${repo.name}/languages`, headers
    );
    if (langRes) {
    ...
    const contentsRes = await safeFetch(
        `https://api.github.com/repos/${repo.owner}/${repo.name}/contents`, headers
    );

    let hasTests = false, hasCI = false, hasDockerfile = false;
    let hasContributing = false, hasLicense = false, hasChangelog = false;
    let hasPrettierOrLint = false, hasGitignore = false;
    let hasEnvExample = false, hasEnvCommitted = false;
    ...
    if (contentsRes) {
        ...
        if (Array.isArray(contents)) {
            ...
            const dirNames = contents.filter((f: any) => f.type === 'dir').map((f: any) => f.name.toLowerCase());
            fileCount = contents.length;

            // Test detection
            hasTests = fileNames.some(f =>
                f.includes('test') || f.includes('spec') || f.includes('__tests__') ||
                f === 'jest.config.js' || f === 'jest.config.ts' || f === 'vitest.config.ts' ||
                f === 'pytest.ini' || f === '.pytest_cache'
            );

            // CI/CD detection
            hasCI = fileNames.some(f =>
                f === '.github' || f === '.circleci' || f === '.travis.yml' ||
                f === 'jenkinsfile' || f === '.gitlab-ci.yml'
            );

            // Docker
            hasDockerfile = fileNames.some(f => f.includes('dockerfile') || f === 'docker-compose.yml' || f === 'docker-compose.yaml');

            // Contributing guide
            hasContributing = fileNames.some(f => f.includes('contributing'));

            // License (REAL detection, not mocked)
            hasLicense = fileNames.some(f => f === 'license' || f === 'license.md' || f === 'license.txt' || f === 'licence' || f === 'licence.md');

            // Changelog
            hasChangelog = fileNames.some(f => f === 'changelog' || f === 'changelog.md' || f === 'changes.md' || f === 'history.md');

            // Linter/formatter
            hasPrettierOrLint = fileNames.some(f =>
                f.includes('eslint') || f.includes('prettier') || f.includes('tslint') ||
                f === '.editorconfig' || f === 'biome.json' || f === '.stylelintrc' ||
                f === 'pylintrc' || f === '.flake8' || f === 'pyproject.toml' ||
                f === '.rubocop.yml'
            );
            ...
            // .env handling
            hasEnvExample = fileNames.some(f => f === '.env.example' || f === '.env.sample' || f === '.env.template');
            hasEnvCommitted = fileNames.some(f => f === '.env' || f === '.env.local' || f === '.env.production');

            // API docs
            hasApiDocs = fileNames.some(f =>
                f === 'swagger.json' || f === 'swagger.yaml' || f === 'openapi.json' || f === 'openapi.yaml' ||
                f.includes('apidoc') || dirNames.includes('docs') || dirNames.includes('documentation')
            );

            // Modular structure: ≥3 distinct top-level directories (excluding meta dirs)
            const meaningfulDirs = dirNames.filter(d =>
                !d.startsWith('.') && d !== 'node_modules' && d !== 'dist' && d !== 'build' && d !== 'out'
            );
    ...
    if (readmeIndex < 8) { // Analyze READMEs for top 8 repos
        const readmeRes = await safeFetch(
            `https://api.github.com/repos/${repo.owner}/${repo.name}/readme`, headers
        );
        if (readmeRes) {
            ...
            if (readmeJson.content) {
                try {
                    ...
                    readmeSnippets[repo.name] = decoded.substring(0, 800) + (decoded.length > 800 ? '...' : '');

                    // Word count
                    readmeWordCount = decoded.split(/\s+/).filter(w => w.length > 0).length;

                    // Check for installation instructions
                    const lowerReadme = decoded.toLowerCase();
                    readmeHasInstallInstructions = /install|setup|getting started|npm install|pip install|yarn add|pnpm add|brew install/i.test(lowerReadme);

                    // Check for usage examples / code snippets
                    readmeHasUsageExamples = /```[\s\S]*?```|usage|example|how to use/i.test(decoded);

                    // Check for screenshots / images
                    readmeHasScreenshots = /!\[.*?\]\(.*?\)|<img\s|screenshot|demo|preview/i.test(decoded);
                } catch { /* Base64 decode failure */ }
            }
        }
    }
